...
export class ProductsService {
  ...
  async findAll() {
    ...
    return products.map((product) => {
      ...
      const additionalProductImages = productImages.filter(
        (img) => !img.isMain
      );

      const variantImagesMap: Record<
        number,
        { main: any | null; additional: any[] }
      > = {};
  ...
  async update(id: number, dto: UpdateProductsDto) {
    ...
    if (sku) {
      ...
      if (dup) {
        throw new BadRequestException(
          `Another product with SKU '${sku}' exists.`
        );
      }
    }
    ...
    const updated = await this.prisma.product.update({
      ...
      data: {
        ...
        ...(sizeId &&
          Number(sizeId) > 0 && { size: { connect: { id: Number(sizeId) } } }),
        ...(colorId &&
          Number(colorId) > 0 && {
            color: { connect: { id: Number(colorId) } },
          }),
  ...
  async remove(id: number) {
    ...
    deleteOps.push(
      this.prisma.productFeature.deleteMany({ where: { productId: id } })
    );
    deleteOps.push(
      this.prisma.productFilter.deleteMany({ where: { productId: id } })
    );
    deleteOps.push(
      this.prisma.variant.deleteMany({ where: { productId: id } })
    );
    deleteOps.push(
      this.prisma.productDetails.deleteMany({ where: { productId: id } })
    );
    deleteOps.push(
...
async getProducts(query: {
  ...
  async getProductDetails(productId: number, variantId?: number) {
    ...
    if (variantId) {
      ...
      variantImages = product.images.filter(
